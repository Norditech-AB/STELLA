...
class SQLite(DatabaseInterface, ABC):
    ...
    def create_user(self, email, password) -> User:
        cursor = self.conn.cursor()
        cursor.execute("INSERT INTO user (email, password, workspaces) VALUES (?, ?, ?)",
                       (email, password, json.dumps([])))
        user_id = cursor.lastrowid
        self.conn.commit()
        return User(user_id=str(user_id), email=email, password=password, workspaces=[])
    ...
    def get_user_workspaces(self, user_id) -> list[str]:
        ...
        return self._deserialize_list(row['workspaces'])
    ...
    def update_user(self, user: User) -> User:
        cursor = self.conn.cursor()
        cursor.execute("UPDATE user SET email = ?, password = ?, workspaces = ? WHERE id = ?", (
            user.email,
            user.password,
